...
! appears only once. The relative order of the elements should be kept the same. Then return the number of unique elements in 
! nums.
...
class Solution {
    public int removeDuplicates(int[] nums) {
        ...
        for(int i=1; i<n; i++){  // starts from i=1 since first element is always unique
            if(nums[i] != nums[i-1]){
                nums[k] = nums[i];
                k++;
            }
