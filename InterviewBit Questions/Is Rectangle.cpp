...
Output Format
If any such rectangle exist whose sides are A, B, C, D in any order then return 1 else return 0.
...
int Solution::solve(int A, int B, int C, int D) {
    ...
    // If the first two or last two side lengths are equal, it's a rectangle
    if ((A==B && C==D) || (A==C && B==D))
        return 1;
    else
        return 0;
